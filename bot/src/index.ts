...
// Optional second "Test" entry → a beta build of the Mini App (e.g. a /beta/
// path). When unset, the console shows only the single "Open" button as before;
// set it to surface a second inline button without touching anything else.
const TEST_MINI_APP_URL = process.env.TEST_MINI_APP_URL || ''
...
// Inline button(s) (inside the message) that launch the Mini App. No emoji.
// A second "Test" row is added only when TEST_MINI_APP_URL is configured.
const consoleKeyboard = {
  inline_keyboard: [
    [{ text: BUTTON, web_app: { url: MINI_APP_URL } }],
    ...(TEST_MINI_APP_URL ? [[{ text: TEST_BUTTON, web_app: { url: TEST_MINI_APP_URL } }]] : []),
  ],
}
