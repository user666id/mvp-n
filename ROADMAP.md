...
- Paid subscriptions & payments: crypto (GRAM / USDT-TON / USDT-TRC20, matched by a unique amount), Telegram Stars, and one-tap TON Connect (returning to the app after signing via `twaReturnUrl`); live GRAM/USD rate; "Subscription" pane — status, renewal and payment history.
...
- Bot: an inline "Open" launcher (chat menu button configured in BotFather), private chats only, language sync with the app.
...
| Feature | Summary |
|------|------|
| RU cards & SBP via Platega | Card / SBP / crypto acquiring for RU users (researched: pay-per-transaction, no monthly minimum, no legal entity required). |
