    ...
    groups: [
      {
        ...
        items: [
          {
            en: 'Configure now opens the setup guide right away — subscription link, QR and copy on top; the install steps, server status and config settings follow in one flow.',
            ru: '«Настроить» сразу открывает инструкцию по подключению — сверху ссылка подписки, QR и копирование, ниже шаги установки, статус сервера и настройки конфига.',
          },
          {
            en: 'A fresh icon set — the app now uses the Telegram iOS interface icons throughout.',
            ru: 'Новые значки — приложение теперь использует фирменные иконки интерфейса Telegram iOS.',
          },
        ],
      },
      {
        ...
        items: [
          {
            en: 'Removed the duplicate copy button from the setup steps — the link capsule on top already covers it.',
            ru: 'Убрана дублирующая кнопка копирования в шагах установки — ссылку можно скопировать в капсуле сверху.',
          },
        ],
      },
    ],
        ...
        groups: [
          {
            ...
            items: [
              {
                en: 'A design tune-up on Apple principles: everything sits on an 8-pixel spacing grid, the type scale is cleaned up, compact rows are the iOS-standard 44px.',
                ru: 'Дизайн подтянут по принципам Apple: всё на сетке отступов 8 пикселей, шкала шрифтов вычищена, компактные строки — стандартные для iOS 44px.',
              },
              {
                en: 'Bottom pop-ups (QR, confirmations, pickers) now float on the same liquid glass as the tab bar and menus.',
                ru: 'Нижние всплывающие окна (QR, подтверждения, выбор значений) теперь парят на том же жидком стекле, что таб-бар и меню.',
              },
            ],
          },
        ],
        ...
        groups: [
          {
            ...
            items: [
              {
                en: 'Admin: key activation right on a profile — enter an access key and the profile gets what it grants (days stack, lifetime clears the limit).',
                ru: 'Админ: активация ключа прямо в профиле — вводишь ключ доступа, и профиль получает то, что он даёт (дни складываются, безлимит снимает срок).',
              },
            ],
          },
          {
            ...
            items: [
              {
                en: 'The OS picker now shows each platform’s own logo — Apple, Finder for macOS, the  tv mark, the Android robot, Windows panes, Tux for Linux.',
                ru: 'В списке ОС теперь фирменные логотипы платформ — яблоко, Finder для macOS, знак  tv, робот Android, панели Windows, Тукс для Linux.',
              },
              {
                en: 'The key icon is redrawn in the passkey style, and key activation looks and reads the same everywhere.',
                ru: 'Значок ключа перерисован в стиле passkey, а активация ключа везде выглядит и называется одинаково.',
              },
            ],
          },
        ],
        ...
        groups: [
          {
            ...
            items: [
              {
                en: 'Apple TV and Android TV in the setup guide — with their own install steps, and OS icons in the picker.',
                ru: 'Apple TV и Android TV в инструкции подключения — со своими шагами установки; в списке ОС появились значки.',
              },
            ],
          },
          {
            ...
            items: [
              {
                en: 'Icon polish: a new key, a theme toggle icon, a location pin for the server IP; the server-stats screen lost its oversized header.',
                ru: 'Доработаны значки: новый ключ, значок темы, булавка у IP сервера; с экрана статистики убран крупный заголовок.',
              },
            ],
          },
        ],
    ...
    groups: [
      {
        ...
        items: [
          {
            en: 'Login and access hardening — extra server-side checks so a misconfigured server can never weaken Telegram sign-in.',
            ru: 'Усилена защита входа и доступа — дополнительные серверные проверки, чтобы неверная настройка сервера не могла ослабить вход через Telegram.',
          },
        ],
      },
      {
        ...
        items: [
          {
            en: 'Screens now scroll as one piece — the avatar and wallet at the top move together with the content instead of staying pinned.',
            ru: 'Экраны теперь листаются одним куском — аватар и кошелёк сверху едут вместе с содержимым, а не остаются на месте.',
          },
        ],
      },
    ],
        ...
        groups: [
          {
            ...
            items: [
              {
                en: 'The device-limit wheel scrolls again — the drag-to-scroll on the screen was grabbing the gesture; now a spin inside the wheel (or any inner scroller) stays with it.',
                ru: 'Колесо лимита устройств снова листается — тянущийся скролл экрана перехватывал жест; теперь прокрутка внутри колеса (и любого вложенного списка) остаётся за ним.',
              },
            ],
          },
        ],
        ...
        groups: [
          {
            ...
            items: [
              {
                en: 'Small consistency polish — empty lists (no devices, no keys, empty history, …) now use one shared style, so spacing and text size match everywhere.',
                ru: 'Небольшая полировка единообразия — пустые списки (нет устройств, нет ключей, пустая история…) теперь в одном общем стиле, с одинаковыми отступами и размером текста.',
              },
            ],
          },
        ],
        ...
        groups: [
          {
            ...
            items: [
              {
                en: 'Navigation is fully instant — tabs and drill-down screens open with no transition at all, so nothing can stutter. (Drag-to-scroll and the bottom pop-ups are unchanged.)',
                ru: 'Навигация полностью мгновенная — вкладки и экраны вглубь открываются без переходов, так что дёргаться нечему. (Листание и всплывающие окна снизу — без изменений.)',
              },
            ],
          },
        ],
        ...
        groups: [
          {
            ...
            items: [
              {
                en: 'Navigation now matches iOS/Telegram — tabs switch instantly, and drill-down screens slide in from the right. The slide was rebuilt to run on the GPU without the stutter it had before.',
                ru: 'Навигация как в iOS/Telegram — вкладки переключаются мгновенно, а экраны вглубь выезжают справа. Слайд пересобран на GPU, без прежних подтормаживаний.',
              },
            ],
          },
        ],
        ...
        groups: [
          {
            ...
            items: [
              {
                en: 'Screens now open and close instantly — the slide/zoom transition (which stuttered on some devices) is turned off, so panels snap open with no lag.',
                ru: 'Экраны теперь открываются и закрываются мгновенно — переход со слайдом/зумом (который подтормаживал на части устройств) отключён, панели открываются без задержки.',
              },
            ],
          },
        ],
        ...
        groups: [
          {
            ...
            items: [
              {
                en: 'One consistent feel across the app — every screen and popup now uses the same motion, and pop-up windows scroll if they’re taller than the screen so nothing gets cut off.',
                ru: 'Единый отклик во всём приложении — все экраны и всплывающие окна используют одну анимацию, а высокие окна прокручиваются, чтобы ничего не обрезалось.',
              },
            ],
          },
        ],
        ...
        groups: [
          {
            ...
            items: [
              {
                en: 'The same drag-to-scroll everywhere — the main tabs (Configs, Payment, Admin) now pull up and down with the avatar and wallet just like the other screens, even when the content fits.',
                ru: 'Одинаковое листание везде — главные вкладки (Конфиги, Оплата, Админка) теперь тянутся вверх-вниз вместе с аватаром и кошельком, как остальные экраны, даже когда контент помещается.',
              },
            ],
          },
        ],
