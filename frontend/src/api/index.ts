...
import type {
  AccessKey, AccessKeyRow, AdminProfile, AuthResult, Config, Device, DomainStatus, Order, PlansResponse, Profile, ServerStats, StarsInvoice, TrafficDay,
} from './types'
...
/** Redeem an access key for a profile (admin): the profile gets whatever the
 *  key grants — N days stacked on the current subscription, or lifetime. */
export function adminApplyKey(id: number, key: string) {
  return request<{ paid_until: string | null; lifetime: boolean }>(
    'POST',
    `/admin/profiles/${id}/apply-key`,
    { key },
  )
}
