...
export async function mockRequest(
  ...
  const apKey = path.match(/^\/admin\/profiles\/([^/]+)\/apply-key$/)
  if (apKey && m === 'POST') {
    const k = String(body?.key ?? '').trim().toUpperCase()
    if (!k) throw { errorCode: 'BAD_REQUEST', message: 'key required' }
    if (k.includes('USED')) throw { errorCode: 'KEY_EXPIRED', message: 'key expired or already used' }
    if (k.length < 6) throw { errorCode: 'KEY_NOT_FOUND', message: 'invalid key' }
    const p = adminProfiles.find((x) => String(x.id) === apKey[1])
    if (p && !p.paid_until) throw { errorCode: 'PROFILE_LIFETIME', message: 'profile already has lifetime access' }
    const base = p?.paid_until ? Math.max(Date.parse(p.paid_until), Date.now()) : Date.now()
    const until = new Date(base + 30 * 86_400_000).toISOString()
    if (p) {
      p.paid_until = until
      p.is_expired = false
    }
    return { paid_until: until, lifetime: false }
  }
