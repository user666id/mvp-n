...
import {
  ...
  Globe, Sliders, Apple, Android, Windows, MacOS, Linux, AppleTv, AndroidTv,
...
type OSKey = 'ios' | 'android' | 'windows' | 'macos' | 'linux' | 'appletv' | 'androidtv'
type StoreLink = { label: string; href: string }

const OS_OPTS: { key: OSKey; label: string; Icon: typeof Apple }[] = [
  { key: 'ios', label: 'iOS', Icon: Apple },
  { key: 'android', label: 'Android', Icon: Android },
  { key: 'windows', label: 'Windows', Icon: Windows },
  { key: 'macos', label: 'macOS', Icon: MacOS },
  { key: 'linux', label: 'Linux', Icon: Linux },
  { key: 'appletv', label: 'Apple TV', Icon: AppleTv },
  { key: 'androidtv', label: 'Android TV', Icon: AndroidTv },
]

/** TV platforms: no deep-link import — the subscription is added on the TV
 *  itself (enter the link or scan its QR from the capsule above). */
const isTvOs = (os: OSKey) => os === 'appletv' || os === 'androidtv'
...
const APPS: { id: string; name: string; stores: Partial<Record<OSKey, StoreLink[]>> }[] = [
  {
    ...
    stores: {
      ...
      // Standalone tvOS app ("Happ - Proxy Utility for TV"), tvOS 17.6+.
      appletv: [{ label: 'App Store', href: 'https://apps.apple.com/us/app/happ-proxy-utility-for-tv/id6748297274' }],
      androidtv: [{ label: 'Google Play', href: 'https://play.google.com/store/apps/details?id=com.happproxy' }],
    },
  },
  {
    id: 'v2raytun',
    ...
    stores: {
      ...
      androidtv: [{ label: 'Google Play', href: 'https://play.google.com/store/apps/details?id=com.v2raytun.android' }],
    },
  },
  {
    id: 'v2rayng',
...
export function ConfigDetailSheet({
  ...
  // Re-detect the OS each time the sheet opens (the guide is now its main content).
  useEffect(() => {
    if (!open) return
    const o = detectOs()
    setOs(o)
  ...
  return (
    <>
      <Sheet open={open} onClose={onClose} title={t('detail.title')} pills>
        ...
        <div className="mb-6 mt-1 flex flex-col items-center text-center">
          ...
          <div className="mt-0.5 text-[13px] text-faint">{configSpecLine(meta)}</div>
        </div>

        {/* connection: AmneziaWG .conf or VLESS "install to app" */}
        {isAwg ? (
          <div className="mb-6">
            <div className="mb-2 flex items-start gap-2 rounded-3xl border border-border bg-surface-sunken px-3 py-2.5">
              <pre className="no-scrollbar min-w-0 flex-1 overflow-x-auto whitespace-pre font-mono text-[12px] leading-snug text-ink">
                {awgConf}
              </pre>
        ...
        {!isAwg && (
          <div className="mb-4 flex items-center gap-2 rounded-3xl border border-border bg-surface px-4 py-3">
        ...
        {/* install guide — OS dropdown + launcher tabs + numbered stepper, all in
            one capsule (used to live behind the "Add to app" button in a separate sheet) */}
        {!isAwg && (
          <div className="mb-4 rounded-3xl border border-border bg-surface p-4">
            {/* OS — dropdown, same component as the language selector */}
            <div className="mb-4">
              <Dropdown
                value={os}
                onChange={(v) => {
                ...
                options={OS_OPTS.map((o) => ({
                  value: o.key,
                  label: o.label,
                  icon: <o.Icon size={18} className="text-muted" />,
                }))}
              />
            </div>
            ...
            {(() => {
              ...
              const isTv = isTvOs(os)
              return (
                <div key={launcher} className="animate-fade">
                  ...
                  <div className="space-y-5">
                    ...
                    <div className="relative flex gap-3.5">
                      ...
                      <div className="min-w-0 flex-1">
                        ...
                        <p className="mt-1 text-[13px] leading-relaxed text-muted">
                          {t(isTv ? 'detail.step1BodyTv' : isMobile ? 'detail.step1Body' : 'detail.step1BodyDesktop')}
                        </p>
                        {store && (
                          <button
                            onClick={() => openLink(store.href)}
                            className="mt-2.5 inline-flex h-10 w-full items-center justify-center gap-1.5 tap rounded-full bg-accent/15 text-[14px] font-medium text-accent active:bg-accent/25"
                          >
                    ...
                    <div className="relative flex gap-3.5">
                      ...
                      <div className="min-w-0 flex-1">
                        ...
                        <p className="mt-1 text-[13px] leading-relaxed text-muted">
                          {t(isTv ? 'detail.step2BodyTv' : 'detail.step2Body')}
                        </p>
                        {!isTv && (
                          <Button
                            stretched
                            className="mt-2.5"
                            onClick={() => openLink(importPage(app.id, link, lang))}
                          >
                            {t('detail.addSub')}
                          </Button>
                        )}
