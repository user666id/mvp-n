...
export function SubscribeSheet({
  ...
  const [consent, setConsent] = useState(false) // immediate-start + waive 14-day withdrawal
    ...
    >
      {step === 'select' && (
        <>
          ...
          <Button onClick={() => setStep('confirm')} stretched disabled={!asset}>
            {t('common.continue')}
          </Button>
        </>
      )}
      ...
      {step === 'confirm' && (
        <div className="flex flex-col">
          ...
          {/* Digital-service consent: immediate start + waiver of the 14-day
              withdrawal right (EU). Gates payment. */}
          <label className="mt-5 flex items-start gap-2.5 px-1 text-left">
            <input
              type="checkbox"
              checked={consent}
            ...
            />
            <span className="text-[12.5px] leading-snug text-muted">{t('pay.withdrawalConsent')}</span>
          </label>
          {isStars ? (
            <Button onClick={payStars} loading={busy} disabled={!consent} stretched className="mt-4">
            ...
            <>
              ...
              <Suspense
                fallback={
                  <Button loading stretched className="mt-4">
                    {t('pay.payWallet')}
                  </Button>
                }
              >
            ...
            <Button onClick={startPay} loading={busy} disabled={!consent} stretched className="mt-4">
              {t('pay.toPayment')}
            </Button>
          )}
        </div>
