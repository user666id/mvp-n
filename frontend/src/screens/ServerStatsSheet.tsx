...
import { Globe, Clock, MapPin } from '../components/icons'
...
function StatsSkeleton() {
  return (
    <div className="animate-fade">
      <div className="skeleton mb-4 h-[52px] w-full rounded-3xl" />
      <div className="skeleton mb-6 h-[150px] w-full rounded-3xl" />
      {[0, 1, 2].map((i) => (
  ...
  return (
    <>
      ...
      <div className="mb-6 overflow-hidden rounded-3xl border border-border bg-surface">
        ...
        {stats.server_ip && <InfoRow icon={<MapPin size={20} />} text={stats.server_ip} />}
...
function InfoRow({
  ...
  return (
    <div
      className={'flex items-center gap-3 px-4 py-3 ' + (last ? '' : 'border-b border-border')}
    >
