...
# Missing `release` (fresh box before the first `main:release` push, or a deleted
# branch) is a graceful no-op, not a hard failure under `set -e`.
git fetch --quiet origin release 2>/dev/null || exit 0
BEFORE="$(git rev-parse HEAD)"
AFTER="$(git rev-parse origin/release 2>/dev/null)" || exit 0
