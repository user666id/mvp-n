...
// A forged blob signed with an empty bot token must never verify, even if the
// server were misconfigured with BOT_TOKEN unset (see config.Load guard).
func TestVerifyInitData_EmptyBotTokenRejected(t *testing.T) {
	params := map[string]string{
		"user":      `{"id":1000000001,"first_name":"Attacker"}`,
		"auth_date": strconv.FormatInt(time.Now().Unix(), 10),
	}
	// Attacker signs with "" — the constant they can reproduce offline.
	initData := encodeInitData(params, signInitData(params, ""))

	if _, err := verifyTelegramInitData(initData, ""); err != errBadInitData {
		t.Fatalf("empty bot token: got err=%v, want errBadInitData", err)
	}
}
