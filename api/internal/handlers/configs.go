...
func (h *Handler) GetByShortID(w http.ResponseWriter, r *http.Request) {
	...
	err := h.DB.QueryRowContext(r.Context(), `
		SELECT vc.client_uuid, vc.location, vc.enhanced, vc.game_mode, vc.protocol, COALESCE(vc.awg_conf, ''),
		       u.paid_until, u.is_blocked
		FROM vpn_configs vc JOIN users u ON u.id = vc.user_id
		WHERE vc.short_id = $1 AND vc.is_active = true AND u.deleted_at IS NULL`, id,
	).Scan(&clientUUID, &location, &enhanced, &gameMode, &protocol, &awgConf, &paidUntil, &userBlocked)
	if err != nil {
		http.NotFound(w, r)
		return
	}
	// Don't serve the config (which discloses the origin server) once access is
	// gone: expired paid subscription (NULL = unlimited key), or a blocked
	// account. The tunnel is already cut server-side; 404 keeps this public URL
	// from leaking the server IP/UUID and lets the client drop the stale sub.
	if userBlocked || (paidUntil.Valid && !paidUntil.Time.After(time.Now())) {
		http.NotFound(w, r)
		return
	}
