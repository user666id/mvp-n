...
└── internal/
    ...
    │   ├── configs.go         # CRUD конфигов, buildURI (VLESS)
...
### Internal (заголовок `X-Internal-Token`, токен на каждый сервис)
```
POST /internal/provision     — выдать per-device конфиг по HWID (зовёт connect; токен INTERNAL_TOKEN_CONNECT)
GET  /internal/user-lang     — язык пользователя (читает бот; токен INTERNAL_TOKEN_BOT)
```
Сравнение токена — constant-time. Оба токена падают на legacy `ADMIN_TOKEN`, если
per-service не заданы.
...
(+ опц. `JWT_SECRET_PREVIOUS` для ротации), `BOT_TOKEN`, `ADMIN_TG_IDS`,
