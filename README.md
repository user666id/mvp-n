...
`.env` (в `.gitignore`) и в GitHub Actions Secrets — в репозитории секретов нет.

## Деплой и бэкап

Деплой: `git push` в `main` → GitHub Actions на VPS запускает
[`scripts/deploy.sh`](./scripts/deploy.sh) — пересобирает только изменившиеся
сервисы, реконсилит стек при изменении compose, синхронизирует собранный Mini App.
...
- [`STATUS.md`](./STATUS.md) · [`ROADMAP.md`](./ROADMAP.md) — статус и план
